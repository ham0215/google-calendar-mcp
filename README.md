...
- 6ヶ月以上経過した場合のみ再認証が必要
- トークンの状態確認: `npm run auth:status`で現在の認証状態を確認可能
...
サーバーは自動的に期限切れトークンを更新します:
- **事前更新**: トークンの有効期限5分前に自動的に更新されます（src/auth/oauth.ts:175-180）
- **詳細ログ**: トークン更新時にログが出力され、次の有効期限を確認できます
- **状態確認**: `npm run auth:status`で現在のトークン状態を確認可能

以下の場合は再認証が必要:
- 6ヶ月以上経過したリフレッシュトークン
- Google Cloudプロジェクトが無効化されている場合
- ユーザーがアプリのアクセス権を取り消した場合
